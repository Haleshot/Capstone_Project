...
# and `pip install Capstone_project` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "Capstone_project", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n Capstone_project -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "Capstone_project", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "Capstone_project", "YOUR_CONSOLE_SCRIPT"]
